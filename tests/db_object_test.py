...
def test_object_name():
    assert (db_object.object_name(course_id=1, ext='.txt',
                                 data=DATA, data_sha256=DATA_SHA256)
            == db_object.object_name(data=DATA, course_id=1, ext='.txt'))
    assert (db_object.object_name(data=DATA,course_id=2, ext='.tcv')
            == f"2/{DATA_SHA256}.tcv")
