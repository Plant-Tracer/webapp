...
- You may use the Marker Table to add, delete, and rename markers. There may be any number of markers. To rename a marker, click the pencil icon (✏️) next to its name.
...
- Markers whose names have the form ``RulerXXmm`` are special. ``XX`` is any non-negative integer representing a real-world distance in millimeters. These markers are intended to be used for distance calibration. If the image has a ruler in it, place one ``RulerXXmm`` marker at a known position on the ruler and another at a second known position. **The value of XX should match the actual ruler markings in your video** — for example, use ``Ruler 0mm`` and ``Ruler 30mm`` if your ruler is 30mm long, not ``Ruler 10mm``. The default markers are named ``Ruler 0mm`` and ``Ruler 10mm``; rename them (using the pencil icon ✏️ in the Marker Table) to match your ruler before tracing. In this way, Plant Tracer can report analysis results in millimeters rather than numbers of pixels in the image.
