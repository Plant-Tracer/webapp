...
- When tracking is complete, you can press the "Download Trackpoints" button to get the tracking data in CSV format. Each position column header notes its unit. ``Ruler XXmm`` marker columns are always in pixels. Other markers' columns are in millimeters when you have calibrated with two or more ``Ruler XXmm`` markers placed on the ruler in your video (moved off their default positions); otherwise they are in pixels.
