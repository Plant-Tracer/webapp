    ...
    with zipfile.ZipFile(zip_path, mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
        ...
        for packet in container.demux(vstream):
            for frame in packet.decode():
                # Stop early if we passed end_time
                ...
                # Correct rotation if present
                degrees = _rotation_degrees_from_frame(frame)
                img = _apply_rotation(img, degrees)

                # Downscale to VGA
                img = _resize_image(img, target_wh=target_wh, mode=mode)
                ...
                # Write directly into the zip (no temp file)
