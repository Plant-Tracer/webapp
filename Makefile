...
merge-coverage:
	...
	@if [ -f coverage/browser-coverage.json ]; then \
		...
		echo Coverage merged and saved to coverage/coverage-final.json; \
		rm -f coverage/browser-coverage.json; \
		echo Removed browser-coverage.json to avoid duplicate processing by codecov; \
	else \
		echo No browser coverage found, using Jest coverage only; \
	fi
