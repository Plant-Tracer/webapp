...
SESSION_TTL_SECS = int(
    os.environ.get("SESSION_TTL_SECS", str(60 * 60 * 24 * 180))
)  # 180 days
dynamodb_client: DynamoDBClient = boto3.client("dynamodb")
dynamodb_resource: DynamoDBServiceResource = boto3.resource(
    "dynamodb", region_name=DDB_REGION
)  # our dynamoDB is in region us-east-1
...
def add_user_log(event, user_id, message, **extra):
    ...
    LOGGER.debug(
        "client_ip=%s user_id=%s message=%s extra=%s",
        client_ip,
        user_id,
        message,
        extra,
    )
    ret = users_table.put_item(
        Item={
            A.USER_ID: user_id,
